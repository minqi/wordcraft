...
class FileWriter:
    def __init__(
        ...
        self.paths = dict(
            ...
            testlogs="{base}/testlogs.csv".format(base=self.basepath),
            test_trainlogs="{base}/test_trainlogs.csv".format(base=self.basepath),
        )
        ...
        self._test_test_logfile = open(self.paths["testlogs"], "a")
        self._test_test_logwriter = csv.DictWriter(self._test_test_logfile, fieldnames=["step", "mean_episode_return"])
        self._wrote_test_test_fields = False

        self._test_train_logfile = open(self.paths["test_trainlogs"], "a")
        self._test_train_logwriter = csv.DictWriter(self._test_train_logfile, fieldnames=["step", "mean_episode_return"])
        self._wrote_test_train_fields = False
    ...
    def log_test_test(self, to_log) -> None:
        if not self._wrote_test_test_fields:
            self._wrote_test_test_fields = True
            self._test_test_logwriter.writeheader()

        self._test_test_logwriter.writerow(to_log)
        self._test_test_logfile.flush()

    def log_test_train(self, to_log) -> None:
        if not self._wrote_test_train_fields:
            self._wrote_test_train_fields = True
            self._test_train_logwriter.writeheader()

        self._test_train_logwriter.writerow(to_log)
        self._test_train_logfile.flush()
